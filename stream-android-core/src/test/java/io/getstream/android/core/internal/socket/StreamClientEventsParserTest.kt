...
class StreamClientEventsParserTest {

    private val jsonSerialization: StreamJsonSerialization = mockk()

    private val parser = StreamClientEventsParser(jsonSerialization)
    ...
    fun `serialize delegates to jsonParser success`() {
        ...
        assertEquals("""{"type":"ok"}""", result.getOrNull())
    }
    ...
    fun `deserialize delegates to jsonParser success`() {
        ...
        assertTrue(result.isSuccess)
        assertSame(event, result.getOrNull())
    }
