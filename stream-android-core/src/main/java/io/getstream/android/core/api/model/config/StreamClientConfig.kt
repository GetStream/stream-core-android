...
/**
 * Aggregates all configuration required to create a [StreamClient][io.getstream.android.core.api.StreamClient].
 *
 * This is a DSL-friendly container that groups concerns into logical sections:
 * [logging], [http], [socket], [processing], and [serialization]. Any field left
 * as `null` (or empty) will be filled with sensible defaults by the client factory.
 *
 * Typical usage:
 * ```
 ...
 * @property tokenManager Optional pre-built [StreamTokenManager]. If null, one is created
 * by the factory using [tokenProvider].
 */
@StreamCoreApi
@StreamDsl
data class StreamClientConfig(
     ...
     * @property connectionIdHolder Optional holder used to inject the current connection id
     * into request headers (if provided). If `null`, a default holder may be created elsewhere.
     */
    data class Http(
     ...
     *
     * @property connectionIdHolder Optional holder for the active connection id, used by
     * WebSocket and HTTP integrations.
     * @property socketFactory Optional custom [StreamWebSocketFactory]. If `null`, a default
     * factory will be used.
     * @property healthMonitor Optional [StreamHealthMonitor] to supervise the socket liveness.
     ...
     * @property eventDebounce Optional debounce processor for inbound raw WebSocket messages.
     * If `null`, a default processor will be used.
     */
    data class Socket(
        val connectionIdHolder: StreamConnectionIdHolder? = null,
     ...
     *
     * You can inject custom implementations for testing or advanced tuning. Any component left
     * as `null` will be replaced with a default one.
     *
     * @property singleFlight Optional [StreamSingleFlightProcessor] to de-duplicate concurrent work.
